...
- (void)yr_viewWillLayoutSubviews
{
    ...
    if (self.navigationController.viewControllers.firstObject != self && [self shouldGenerateNavigationBarImageView]) {
        CGRect frame = self.navigationController.navigationBar.visibleBoundry;
        frame.origin.y -= UINavigationBarHeight;
        UIImageView *navigationBarSnapshot = [[UIImageView alloc] initWithFrame:frame];
